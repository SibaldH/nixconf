      ...
      packages.niri = inputs.wrapper-modules.wrappers.niri.wrap {
        ...
        settings =
          ...
          in
          {
            ...
            binds = {
              "Mod+Return" = _: {
                props.hotkey-overlay-title = "Open a Terminal: ${self'.packages.kitty.pname}";
              ...
              "Mod+E" = _: {
                props.hotkey-overlay-title = "Open Yazi";
                content.spawn = [
                  "${lib.getExe self'.packages.kitty}"
                  "--hold"
                  "${lib.getExe self'.packages.yazi}"
                ];
              };

              "Mod+Shift+Q" = _: {
                props.hotkey-overlay-title = "Lock screen";
                content.spawn-sh = "${noctaliaExe} ipc call lockScreen lock";
              };
              ...
              "XF86AudioPlay" = _: {
                props.allow-when-locked = true;
                content.spawn-sh = "${pkgs.playerctl} play-pause";
              };
              "XF86AudioStop" = _: {
                props.allow-when-locked = true;
                content.spawn-sh = "${pkgs.playerctl} stop";
              };
              "XF86AudioPrev" = _: {
                props.allow-when-locked = true;
                content.spawn-sh = "${pkgs.playerctl} previous";
              };
              "XF86AudioNext" = _: {
                props.allow-when-locked = true;
                content.spawn-sh = "${pkgs.playerctl} next";
              };
